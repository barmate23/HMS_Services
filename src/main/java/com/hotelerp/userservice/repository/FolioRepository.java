...
public interface FolioRepository extends JpaRepository<Folio, Long> {
    ...
    @org.springframework.data.jpa.repository.Query(
            "SELECT f FROM Folio f JOIN f.reservation r " +
            "WHERE f.isDeleted = false " +
            "AND f.status.code = 'OPEN' " +
            "AND :today BETWEEN r.checkInDate AND r.checkOutDate")
    java.util.List<Folio> findAllOpenFolios(@org.springframework.data.repository.query.Param("today") java.time.LocalDate today);
